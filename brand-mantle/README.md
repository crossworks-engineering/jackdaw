...
The official **Mantle** logo files — the brain's brand, not this repo's. Jackdaw's
own marks are in [`../brand/`](../brand/).

These are a **copy**. The source of truth is
[`brand/` in the mantle repo](https://github.com/crossworks-engineering/mantle/tree/main/brand);
if the mark changes it changes there first. They are kept here because the
clients still show the Mantle mark where they are talking about the brain
itself. Take the logo from here rather than exporting a fresh one or reusing a
