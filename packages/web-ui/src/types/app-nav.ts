// Contract-next: mantle's next release publishes this module as
// `@mantle/client-types/app-nav` (plus `color` on AppRow and the new
// ProfilePreferences keys). Until the pin carries it, this is a verbatim local
// copy, plus the AppRowWithColor stand-in at the bottom. After the pin bump,
// replace this whole file with:
//   export * from '@mantle/client-types/app-nav';
// and swap AppRowWithColor for the contract's AppRow at its call sites.
/**
...
/** Contract-next stand-in: the published AppRow gains `color`. Optional here
 *  because a brain on an older release doesn't send it. */
export type AppRowWithColor = AppRow & { color?: AppTint | null };
